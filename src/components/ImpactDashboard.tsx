...
export default function ImpactDashboard({ userProfile }: ImpactDashboardProps) {
  ...
  const loadMyStats = async () => {
    ...
    try {
      const { data } = await supabase
        .from('note_impacts')
        .select('co2_saved_kg, water_saved_liters')
  ...
  return (
    <div className="max-w-lg mx-auto px-4 pb-6 space-y-4">
      {/* Mastery card */}
      <div className="bg-gradient-to-b from-sage-100 to-sage-200 rounded-4xl p-6 shadow-soft text-center">
      ...
      {/* Streak card */}
      <div className="bg-white rounded-3xl shadow-card px-5 py-4 flex items-center justify-between">
